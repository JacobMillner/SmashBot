...
module.exports.Housekeeping = function (callback) {
    ...
    if (!usersTableCheck['count(*)']) {
        ...
        sql.prepare("CREATE TABLE Users (id INTEGER PRIMARY KEY, UserName TEXT, DiscordName TEXT, Elo INTEGER, FriendCode TEXT, DateCreated DATETIME default current_timestamp);").run();
    ...
    if (!rankingTableCheck['count(*)']) {
        // If the table isn't there, create it and setup the database correctly.
        sql.prepare("CREATE TABLE Rankings (id INTEGER PRIMARY KEY, Winner_Id INTEGER, Loser_Id INTEGER, RankedMatchFlag INTEGER, DateCreated DATETIME default current_timestamp);").run();
        // Ensure that the "id" row is always unique and indexed.
